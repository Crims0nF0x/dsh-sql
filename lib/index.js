    ...
    for (const definition of tools) {
        if (definition.name === 'sql_exec' && cfg.writeApproval) {
            ...
            definition.gate = async (exec, next) => {
                ...
                const execRecord = (typeof exec === 'object' && exec !== null ? exec : {});
                const args = (typeof execRecord.args === 'object' && execRecord.args !== null ? execRecord.args : {});
                ...
                if (outcome === 'allowed-once')
                    return originalGate ? originalGate(exec, next) : next();
                if (outcome === 'cancelled')
                    return { kind: 'deny', reason: '写操作确认被取消，SQL 未执行。' };
                if (outcome === 'unavailable')
                    return { kind: 'deny', reason: '写操作确认不可用（没有可用的审批界面），SQL 未执行。' };
                return {
                    kind: 'deny',
